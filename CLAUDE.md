# Kaachan

## Project
This repository implements Kaachan, a design linter for Slime Architecture.
For project overview, architecture, and implementation plan, read **README.md** and **docs/**.

## Documentation
- `docs/overview.md` — project overview and Lv1–Lv10 architecture table
- `docs/implementation-plan.md` — implementation phases, types, CLI interface, project structure
- `docs/kaachan-design.md` — Kaachan linter specification (rules, thresholds, detection strategies)

Theory source of truth: https://github.com/u83ism/kaachan-and-slime-docs

## Code Style
See `.claude/rules/code-style.md`.

## Communication
- Always respond in the same language as the user's input.
- Code, comments, and documentation must remain in English regardless of the conversation language.
